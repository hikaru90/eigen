...
- **QA agent has verified the feature** (see Automatic QA Testing below).
...
## Automatic QA Testing (Non-Negotiable)

**After completing any feature implementation, you MUST trigger the QA agent to verify your work.** This is part of the Definition of Done.

### When to Trigger
- After you finish implementing a feature
- All code changes are done
- You've done a basic sanity check (it runs, no obvious errors)

### How to Trigger

Run this command (replace bracketed values):

```bash
pi --skill qa-feature --no-session --print "Test the following feature: [FEATURE_DESCRIPTION]. Changed files: [LIST_FILES]. Acceptance criteria: [CRITERIA]"
```

**Example:**
```bash
pi --skill qa-feature --no-session --print "Test the following feature: Thought Capture Flow. User can submit a raw thought, it gets stored, and sees confirmation with metadata. Changed files: src/routes/capture/+page.svelte, src/lib/server/ingest.ts. Acceptance criteria: 1) Empty submission shows validation error 2) Valid submission shows success toast 3) Thought appears in list within 2 seconds"
```

### Handling QA Results

The QA agent returns a JSON verdict:

- **`verdict: "ship_it"** → Feature is ready. Report success to user.
- **`verdict: "needs_fixes"** → Fix the issues listed, then re-run QA.
- **`verdict: "redesign_needed"** → Stop. Report to user. This needs human intervention.

### Retry Loop (Max 3 Attempts)

```
Attempt 1: Implement → QA
  └─ If needs_fixes → Attempt 2: Fix issues → QA
       └─ If needs_fixes → Attempt 3: Fix issues → QA
            └─ If still failing → STOP. Report to user with all QA feedback.
```

**Do NOT exceed 3 QA attempts.** After 3 failures, stop and report to the user with:
- Summary of what was attempted
- All QA feedback collected
...
When QA fails after max attempts:
```
## ❌ QA Failed (3/3 attempts)
