...
<div
	...
	aria-atomic="true"
>
	<CaptureStepRing total={total} completed={completedSteps} active={ringActive} />
	<div class="min-w-0 flex-1">
		<p class="text-sm font-medium text-foreground truncate leading-snug">{statusText}</p>
		{#if offline && (processing || pendingCount > 0)}
			<p class="text-xs text-muted-foreground truncate leading-snug mt-0.5">Offline — will resume when connected</p>
		...
		{:else if !processing && pendingCount > 1}
			<p class="text-xs text-muted-foreground truncate leading-snug mt-0.5">Processing in background</p>
		{/if}
	</div>
</div>
