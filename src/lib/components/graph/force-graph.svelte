<script lang="ts">
  ...
  onMount(() => {
    ...
    ;(async () => {
      ...
      const svg = d3.select(rootEl).append('svg').attr('class', 'block h-full w-full touch-none')
      ...
      applyPresentationFrame = (t: number) => {
        ...
        nodeSel.each(function (d) {
          ...
          inner.attr('opacity', at > 0 ? rt : 1).attr('transform', `scale(${pop * overshoot})`)
          ...
          inner.select('.fg-node-core').attr('fill', revealFlash > 0.02 ? '#22E876' : nodeFill(d))
      ...
      const ro = new ResizeObserver(() => {
        ...
        const grew = d.w > dims.w * 1.35 || d.h > dims.h * 1.35 || (dims.h <= 80 && d.h > 80)
        if (grew) {
