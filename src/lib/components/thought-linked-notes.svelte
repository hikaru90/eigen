    ...
    <ul class="space-y-1">
      {#each files as file (file.id)}
        <li>
          ...
          >
            <FileText class="text-muted-foreground mt-0.5 size-3 shrink-0" aria-hidden="true" />
            <span class="min-w-0">
              <span class="text-foreground block text-[11px] font-medium leading-tight">
                {file.title || m.notes_untitled()}
              </span>
              <span
                class="text-muted-foreground line-clamp-2 block whitespace-pre-wrap text-[10px] leading-snug"
              >
                {file.preview}
              </span>
            </span>
