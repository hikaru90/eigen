<script lang="ts">
  ...
  const balanceUsd = $derived(formatCreditsAsUsd(availableCredits))
  ...
  async function setupPayPal() {
    ...
    topUpError = null
    try {
      teardownPayPal = await initPayPalCheckout({
  ...
  {#if !compact}
    <div>
      ...
      <p class="text-3xl font-semibold tracking-tight tabular-nums">
        {formatWalletBalance(availableCredits)}
  ...
  {:else}
    <p class="text-muted-foreground text-xs tabular-nums">
      ...
      <span class="text-foreground font-medium"
        >{formatWalletBalance(availableCredits)} credits</span
      >
    ...
    <div class="space-y-3">
      <div>
        ...
        <p class="text-muted-foreground mt-0.5 text-xs">
          Pay in USD. Credits are added to your wallet immediately after checkout.
        </p>
      </div>
      ...
      {:else}
        <p class="text-destructive text-xs">
          PayPal is not configured on this deployment. Contact the operator to enable credit
          top-ups.
        </p>
      {/if}
      {#if topUpStatus}
