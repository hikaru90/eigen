<script lang="ts">
  ...
  let view = $state<CurrentUserView>(getCurrentUserView())

  onMount(() => {
    return subscribeCurrentUserView((v) => {
      view = v
    })
  })

  function select(next: CurrentUserView) {
    setCurrentUserView(next)
    open = false
  }
...
<Popover.Root bind:open>
  ...
  >
    ...
    <span
      ...
      title={triggerLabel}>{triggerLabel}</span
    >
    <ChevronDown
      class="size-4 shrink-0 text-[#111] dark:text-white"
      strokeWidth={2}
      aria-hidden="true"
    />
  </Popover.Trigger>
  ...
  >
    ...
    {#each agentLayers as layer (layer.key)}
      <button type="button" class={optionClass(layer.key)} onclick={() => select(layer.key)}>
        <span class="text-[var(--color-eigen-green)]"
          ><AuthorLayerIcon kind="agent" size="sm" /></span
        >
