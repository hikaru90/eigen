...
<div class="space-y-6">
  <div class="flex flex-wrap items-center justify-between gap-3">
    <p class="text-muted-foreground max-w-xl text-sm">
      Each test ingests captures, runs deterministic structural checks, optionally retrieval/edit, then judges the answer.
      Use <strong>Run</strong> on a catalog row to execute one question at a time.
    </p>
      ...
      <Card.Content class="space-y-4">
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <Button type="button" variant="outline" size="sm" onclick={addCaptureRow}>Add thought</Button>
          </div>
          <p class="text-muted-foreground text-xs">
            Fixture IDs are generated from the thought text when you save (e.g. ec_marcus_walnut_allergy).
          </p>
          {#each formCaptures as cap, i}
            <div class="space-y-2 rounded-md border p-3">
              ...
              <label class="grid gap-1 text-sm">
                ...
                <Textarea bind:value={cap.rawText} rows={2} placeholder="Marcus is allergic to walnuts…" />
              </label>
        ...
        <label class="grid gap-1 text-sm">
          <span class="text-muted-foreground">Retrieval relevant (one per line: fixture_id:grade 0–3)</span>
        ...
        <label class="grid gap-1 text-sm">
          ...
          <span class="text-muted-foreground text-xs">
            Keys: graph, relations, entities, ontology, extraction, embedding, retrieval, learning. Empty
            object uses defaults (graph, embedding, ontology, enrichment).
          </span>
        </label>
        <div class="grid gap-2 rounded-md border p-3">
          ...
          <label class="grid gap-1 text-sm">
            ...
            >
              ...
              {#each formCaptures as cap, i}
                {#if cap.rawText.trim()}
                  <option value={i}>
                    {#if cap.fixtureId.trim()}{cap.fixtureId} — {/if}{excerpt(cap.rawText, 56)}
                  </option>
                {/if}
              {/each}
            </select>
          ...
          <label class="grid gap-1 text-sm">
            ...
            <Textarea bind:value={formEditRawText} rows={2} placeholder="Full revised thought body…" />
          </label>
        </div>
        {#if error}
  ...
  <Card.Root>
    ...
    <Card.Content class="space-y-3">
      {#if items.length === 0}
        <p class="text-muted-foreground text-sm">No questions yet. Create one to run from the Runs tab.</p>
      {:else}
        {#each items as item (item.id)}
          <article class="rounded-lg border p-4">
            <div class="mb-2 flex flex-wrap items-start justify-between gap-2">
              ...
              <div class="flex gap-2">
                {#if onRunQuestion}
                  <Button size="sm" disabled={saving || !isItemActive(item)} onclick={() => void onRunQuestion(item.id)}>
                    Run
                  </Button>
                {/if}
            ...
            <p class="text-muted-foreground mt-0.5 text-sm whitespace-pre-wrap">{item.acceptance}</p>
