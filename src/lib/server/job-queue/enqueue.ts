...
export type EnqueueUserJobResult =
  { enqueued: true; jobId: string } | { enqueued: false; reason: 'duplicate' }
...
export async function enqueueUserJob(input: EnqueueUserJobInput): Promise<EnqueueUserJobResult> {
  ...
  try {
    ...
    if (
      message.includes('user_job_queue_active_dedupe_uidx') ||
...
export function hasActiveJobForUser(userId: string, jobType: UserJobType): Promise<boolean> {
  const sql = createAdminSql(1)
  return (async () => {
    try {
      const db = drizzle(sql, { schema: { userJobQueue } })
