/**
 * Server-side embedding projection utilities.
 * These are separate from the client-side ones to avoid bundling UMAP on the client.
 */
...
export function l2NormalizeEmbeddings(items: { embedding: number[] }[]): number[][] {
  return items.map((i) => {
