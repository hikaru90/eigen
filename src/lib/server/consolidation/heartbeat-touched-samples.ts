...
export async function loadTopCommunitySamples(
	...
	const summaries = await db
		...
		.where(
			and(eq(communitySummary.userId, userId), inArray(communitySummary.communityId, ids))
		);
	const titleById = new Map(
		summaries.map((s) => [s.communityId, s.summaryShort?.trim() || null])
	);
	...
	const memberRows = await db
		...
		.where(
			and(eq(communityMember.userId, userId), inArray(communityMember.communityId, ids))
		);
	...
	return communities.map((c) => {
		...
		const preview = title || (labels.length > 0 ? labels.join(', ') : `Community ${c.id.slice(0, 8)}`);
		return {
			kind: 'note' as const,
			id: c.id,
			label: preview,
