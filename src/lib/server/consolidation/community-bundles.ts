...
export async function loadCommunityThoughtIds(
  ...
  const memberEntityIds = await db
    ...
    .where(and(eq(communityMember.userId, userId), eq(communityMember.communityId, communityId)))
  ...
  const rows = await db
    ...
    .where(and(eq(thoughtEntity.userId, userId), inArray(thoughtEntity.entityId, entityIds)))
    .orderBy(sql`${thoughtEntity.salience} DESC`, sql`${thought.createdAt} DESC`)
    .limit(limit * 2)
...
export async function buildCommunityBundle(userId: string, communityId: string): Promise<boolean> {
  const db = getDb()
...
export async function buildAllCommunityBundles(
  userId: string,
): Promise<CommunityBundleBuildResult> {
  const db = getDb()
  const communities = await db
    ...
    .where(
      and(
        eq(graphCommunity.userId, userId),
        sql`${graphCommunity.level} >= ${COMMUNITY_MID_LEVEL}`,
      ),
    )
