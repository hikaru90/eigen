...
function degreeByEntityFromEdges(edges: Array<{ sourceId: string; targetId: string }>): Map<string, number> {
...
async function loadCoMentionThoughts(userId: string): Promise<CoMentionThought[]> {
	...
	const rows = await db
		...
		.innerJoin(thought, and(eq(entityResolutionLog.thoughtId, thought.id), eq(thought.userId, userId)))
		.where(and(eq(entityResolutionLog.userId, userId), isNotNull(entityResolutionLog.canonicalEntityId)));
