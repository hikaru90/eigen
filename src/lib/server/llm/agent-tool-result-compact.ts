  ...
  return {
    ...
    temporalStatus:
      ...
      row.temporalStatus === 'expired'
        ? row.temporalStatus
        : undefined,
    temporalSummary: typeof row.temporalSummary === 'string' ? row.temporalSummary : undefined,
  }
}
...
export function compactToolResultForLlm(
  ...
  if (tool === 'answer_question') {
    const compact: Record<string, unknown> = {
      answer: typeof obj.answer === 'string' ? snippet(obj.answer, 2_000) : undefined,
...
function compactRetrievedThoughtsForPreview(
  retrieved: unknown,
