...
export async function processWebhookDeliveryJob(job: UserJobQueue): Promise<void> {
  const deliveryId = typeof job.payload.deliveryId === 'string' ? job.payload.deliveryId.trim() : ''
  if (!deliveryId) {
  ...
  const signatureMode: SignatureMode =
    ((ctx as Record<string, unknown>).signature_mode as SignatureMode) || 'generic'
...
export async function disableConnectedAgent(agentId: string): Promise<void> {
  ...
  try {
    ...
    await db.update(connectedAgent).set({ enabled: false }).where(eq(connectedAgent.id, agentId))
  } finally {
    await sql.end()
  }
}
