...
function sqlTextFromExecuteArg(arg: unknown): string {
  ...
  if (arg && typeof arg === 'object') {
    ...
    if (Array.isArray(record.queryChunks)) {
      return record.queryChunks.flatMap((chunk) => chunk.value ?? []).join('')
    }
  }
  return String(arg)
}
...
describe('graph adapter (Apache AGE)', () => {
  ...
  it('tenant-keyed MERGE patterns include user_id in node identity for upsert writes', async () => {
    mockCypherRows([{ id: 't1' }])
    await upsertThoughtNode({ id: 't1', userId: 'u1', category: 'thought' })
    expect(latestCypherCallSql()).toMatch(/MERGE \(t:Thought \{id: 't1', user_id: 'u1'\}\)/)
    ...
    expect(latestCypherCallSql()).toMatch(/MERGE \(e:Entity \{id: 'e1', user_id: 'u1'\}\)/)
    ...
    expect(latestCypherCallSql()).toMatch(/MERGE \(e:Event \{id: 'ev1', user_id: 'u1'\}\)/)
