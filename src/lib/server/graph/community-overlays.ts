...
export async function fetchGraphCommunityOverlays(
  userId: string,
): Promise<GraphCommunityOverlay[]> {
  const db = getDb()
  ...
  const summaryByCommunity = new Map(
    summaries.map(
      (row) => [row.communityId, { short: row.summaryShort, text: row.summaryText }] as const,
    ),
  )
  const labelByEntity = new Map(entities.map((row) => [row.id, row.label] as const))

  return communities
    .filter((c) => isCommunityDbLevel(c.level))
    .map((c) => {
