...
describe('classifyThoughtTemporalStatus', () => {
	...
	it('returns expired when all events ended', () => {
		expect(classifyThoughtTemporalStatus([event], new Date('2026-06-05T00:00:00.000Z'))).toBe('expired');
	});
...
describe('formatTemporalAnnotation', () => {
	...
	it('formats expired events with as-of date', () => {
		...
		const annotation = formatTemporalAnnotation(events, 'expired', new Date('2026-06-05T00:00:00.000Z'));
	...
	it('formats active events without as-of suffix', () => {
		...
		const annotation = formatTemporalAnnotation(events, 'active', new Date('2026-05-28T12:00:00.000Z'));
