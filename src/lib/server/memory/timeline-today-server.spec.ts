...
describe('isOpenTodoToday', () => {
  ...
  it('counts unscheduled open loops as today todo', () => {
    ...
    expect(
      isOpenTodoToday(item({ startAt: null, endAt: null, itemType: 'task' }), now, timeZone),
    ).toBe(true)
  })
...
describe('filterOpenTodoTodayItems', () => {
  ...
  it('merges prior-day overdue into today todo count', () => {
    ...
    expect(
      filterOpenTodoTodayItems([overdueYesterday, upcoming], now, timeZone).map((i) => i.id),
    ).toEqual(['soon', 'yesterday'])
  })
})
