...
describe('computeFocusRank', () => {
	...
	it('ranks items within the week above items further out', () => {
		...
		expect(computeFocusRank(withinWeek, NOW, TZ)).toBeLessThan(computeFocusRank(beyondWeek, NOW, TZ));
	});
	...
	it('boosts deadlines and appointments relative to other kinds', () => {
		...
		expect(computeFocusRank(appointment, NOW, TZ)).toBeLessThan(computeFocusRank(milestone, NOW, TZ));
