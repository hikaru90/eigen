...
export async function updateCanonicalEntityForUser(
  ...
  | { ok: true; entity: Awaited<ReturnType<typeof getCanonicalEntityForUser>> }
 ...
 */
export async function repairCanonicalEntityTypesForUser(userId: string): Promise<{
  repaired: number
  ...
  const stale = await getDb()
    ...
    .where(
      and(
        ...
        notInArray(canonicalEntity.entityType, activeTypeList),
      ),
    )
...
export async function pruneCanonicalEntitiesWithNoThoughtLinks(
  ...
  const uniqueIds = [
    ...new Set(candidateEntityIds.map((id) => id.trim()).filter((id) => id.length > 0)),
  ]
...
export async function consolidateCanonicalEntityAliasesForUser(
  ...
  const available = rows.filter((row): row is typeof row & { embedding: number[] } =>
  ...
  for (const row of available) {
    ...
    if (
      ...
      nearest.distance > DEDUP_DISTANCE_THRESHOLD
    ) {
      continue
    }
