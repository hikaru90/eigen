...
const { getDbMock, loadCtxMock, shouldJudgeMock, judgeMock, promoteTypeMock, ensureMock } =
  vi.hoisted(() => ({
    getDbMock: vi.fn(),
    loadCtxMock: vi.fn(),
...
describe('promoteEntityToProject', () => {
  ...
  it('returns true when entity already has projectStatus', async () => {
    ...
    const select = vi.fn(() => ({ from }))
    getDbMock.mockReturnValue({ select })
    await expect(promoteEntityToProject({ userId: 'u1', entityId: 'e1' })).resolves.toBe(true)
    expect(judgeMock).not.toHaveBeenCalled()
  })

  it('returns false when judgment context is missing', async () => {
    loadCtxMock.mockResolvedValue(null)
    await expect(promoteEntityToProject({ userId: 'u1', entityId: 'e1' })).resolves.toBe(false)
  })
