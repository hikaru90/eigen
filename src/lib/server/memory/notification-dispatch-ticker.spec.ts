...
describe('startNotificationDispatchTicker', () => {
	beforeEach(() => {
		...
		vi.clearAllMocks();
		vi.useFakeTimers();
		tickMock.mockResolvedValue({ eventReminders: { sent: 0, failed: 0 }, dailySummaries: { sent: 0, failed: 0 } });
	});
	...
	it('starts once, runs immediately, and schedules interval', async () => {
		...
		const { startNotificationDispatchTicker, NOTIFICATION_DISPATCH_TICK_MS } = await import(
			'./notification-dispatch-ticker'
		);
