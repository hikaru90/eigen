...
export async function pruneUnusedOntologyEntityKinds(
  ...
  const usedCategoryKeys = new Set(
    categoryRows.map((r) => r.key.trim()).filter((k) => k.length > 0),
  )

  const entitiesToDelete = entities.filter(
    (e) => !protectedEntityKeys.has(e.key) && !usedIds.has(e.id) && !usedCategoryKeys.has(e.key),
  )
    ...
    await tx
      ...
      .where(
        and(
          ...
          inArray(ontologyEntityKind.id, entityIdsToDelete),
        ),
      )
  })
