...
function rowEntity(
  p: Partial<OntologyEntityKindRow> & Pick<OntologyEntityKindRow, 'id' | 'key'>,
...
function rowRel(
  p: Partial<OntologyRelationKindRow> & Pick<OntologyRelationKindRow, 'id' | 'key'>,
): OntologyRelationKindRow {
  return {
    userId: 'u1',
    meaning: '',
...
describe('loadOntology validators', () => {
  ...
  it('validateRelationKindForNewIngest checks active and endpoint ids', () => {
    ...
    expect(
      validateRelationKindForNewIngest(loaded, {
        relationKey: 'rel',
        fromEntityKindId: e1.id,
        toEntityKindId: e2.id,
      }),
    ).toBe(true)
    expect(
      validateRelationKindForNewIngest(loaded, {
        relationKey: 'rel',
        fromEntityKindId: e2.id,
        toEntityKindId: e1.id,
      }),
    ).toBe(false)
    const inactive = buildLoaded([e1, e2], [{ ...rel, active: false }])
    expect(
      validateRelationKindForNewIngest(inactive, {
        relationKey: 'rel',
        fromEntityKindId: e1.id,
        toEntityKindId: e2.id,
      }),
    ...
    const loaded = buildLoaded(
      ...
      [
        rowRel({
          ...
          key: 'on',
          active: true,
          fromOntologyEntityKindId: e.id,
          toOntologyEntityKindId: e.id,
        }),
        rowRel({
          id: 'r2',
          key: 'off',
