...
const DEFAULT_RELATION_KINDS: { key: string; meaning: string; fromKey: string; toKey: string }[] = [
  {
    key: 'leads_to',
    meaning: 'An idea crystallizes into an action',
    fromKey: 'idea',
    toKey: 'task',
  },
  ...
  {
    key: 'informs',
    meaning: 'What you notice shapes choices',
    fromKey: 'observation',
    toKey: 'decision',
  },
  {
    key: 'supports',
    meaning: 'Facts or data back a decision',
    fromKey: 'reference',
    toKey: 'decision',
  },
  {
    key: 'triggered_by',
    meaning: 'A feeling arises from something noticed',
    fromKey: 'feeling',
    toKey: 'observation',
  },
  {
    key: 'recalls',
    meaning: 'A memory evokes an emotional response',
    fromKey: 'memory',
    toKey: 'feeling',
  },
  ...
  {
    key: 'resolves',
    meaning: 'Completing work answers a question',
    fromKey: 'task',
    toKey: 'question',
  },
  {
    key: 'clarifies',
    meaning: 'Introspection resolves a question',
    fromKey: 'reflection',
    toKey: 'question',
  },
  {
    key: 'emerges_from',
    meaning: 'A decision crystallizes into a commitment',
    fromKey: 'decision',
    toKey: 'goal',
  },
]
...
export const DEFAULT_THOUGHT_CATEGORY_KIND_KEYS: readonly string[] =
  DEFAULT_THOUGHT_CATEGORY_KINDS.map((e) => e.key)
  ...
  const thoughtKindRows = await db
    ...
    .where(
      and(
        ...
        eq(ontologyEntityKind.kindType, 'thought_category'),
      ),
    )
...
export async function ensureEntityTypeKindsSeeded(db: AppDatabase, userId: string): Promise<void> {
  ...
  const hasEntityTypes = await db
    ...
    .from(ontologyEntityKind)
    .where(
      and(eq(ontologyEntityKind.userId, userId), eq(ontologyEntityKind.kindType, 'entity_type')),
    )
    .limit(1)
 ...
 */
export async function ensureCriticalEntityTypeKindsActive(
  db: AppDatabase,
  userId: string,
): Promise<void> {
  for (const kind of CRITICAL_ENTITY_TYPE_KINDS) {
    ...
    const [existing] = await db
      ...
      .from(ontologyEntityKind)
      .where(and(eq(ontologyEntityKind.userId, userId), eq(ontologyEntityKind.key, kind.key)))
      .limit(1)
