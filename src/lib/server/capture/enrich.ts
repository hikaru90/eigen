...
export function scheduleEnrichThought(
  ...
  void withDbUser(userId, () => enrichThought(userId, thoughtId, normalizedText, options)).catch(
    (err) => {
...
export function scheduleReenrichThought(
  ...
  void withDbUser(userId, () => reenrichThought(userId, thoughtId, normalizedText, options)).catch(
    (err) => {
...
export async function enrichThought(
  ...
  const results: PromiseSettledResult<void>[] = [entitiesResult, temporalStepResult, metadataResult]
  let allOk = true
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (r.status === 'rejected') {
      ...
      if (stepNames[i] === 'entities') {
        try {
          await db.update(thought).set({ enrichedAt: null }).where(eq(thought.id, thoughtId))
        } catch (clearErr) {
    ...
    try {
      await time('ontology_eval', () =>
        maybeRefreshUserOntology({
          ...
          thoughtCountAfterInsert,
          onBeforeEval: async () => {
            await onProgress?.({ parallel: false, phase: 'ontology_eval' })
          },
        }),
      )
    } catch (err) {
  ...
  if (allOk) {
    ...
    try {
      await time('mark_enriched', () =>
        db.update(thought).set({ enrichedAt: new Date() }).where(eq(thought.id, thoughtId)),
      )
    } catch (err) {
