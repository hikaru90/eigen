...
const {
  ...
  maybeNotifyGroundingQuestionPushMock,
} = vi.hoisted(() => ({
  getDbMock: vi.fn(),
  ...
  maybeNotifyGroundingQuestionPushMock: vi.fn(),
}))
...
vi.mock('$lib/server/grounding/notify-question', () => ({
  maybeNotifyGroundingQuestionPush: maybeNotifyGroundingQuestionPushMock,
}))
...
describe('enrichThought', () => {
  beforeEach(() => {
    ...
    maybeNotifyGroundingQuestionPushMock.mockResolvedValue(undefined)
  ...
  it('notifies the grounding check-in push at the capture milestone', async () => {
    const db = makeDb()
    getDbMock.mockReturnValue(db)

    await enrichThought('u1', 't1', 'hello', { thoughtCountAfterInsert: 20 })

    expect(maybeNotifyGroundingQuestionPushMock).toHaveBeenCalledWith('u1', 20)
  })

  it('does not notify the grounding check-in push without a capture milestone', async () => {
    const db = makeDb()
    getDbMock.mockReturnValue(db)

    await enrichThought('u1', 't1', 'hello')

    expect(maybeNotifyGroundingQuestionPushMock).not.toHaveBeenCalled()
  })

  it('continues enriching when the grounding check-in notify fails', async () => {
    const db = makeDb()
    getDbMock.mockReturnValue(db)
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined)
    maybeNotifyGroundingQuestionPushMock.mockRejectedValue(new Error('notify boom'))

    await enrichThought('u1', 't1', 'hello', { thoughtCountAfterInsert: 10 })

    expect(maybeNotifyGroundingQuestionPushMock).toHaveBeenCalledWith('u1', 10)
    expect(errorSpy).toHaveBeenCalledWith(
      '[enrich] grounding question notify failed',
      expect.objectContaining({ thoughtId: 't1', message: 'notify boom' }),
    )
