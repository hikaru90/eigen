...
describe('enrichQueuedThought', () => {
	beforeEach(() => {
		...
		encryptTenantValueMock.mockImplementation(async ({ plaintext }: { plaintext: string }) => `enc:${plaintext}`);
	...
	it('decrypts encrypted raw/normalized text before enriching', async () => {
		const db = makeDb([
			...
			[{ n: 1 }],
			[{ enrichedAt: new Date('2026-06-01T00:05:00.000Z') }],
			[{ normalizedText: 'secret normalized', category: 'task', memoryType: 'episode', enrichedAt: null }]
		]);
		getDbMock.mockReturnValue(db);
		applyCaptureContentSplitIfNeededMock.mockImplementation(
			async ({ rawText }: { rawText: string }) => ({ rawText, normalizedText: 'secret normalized' })
		);
		...
		expect(applyCaptureContentSplitIfNeededMock).toHaveBeenCalledWith(
			expect.objectContaining({ rawText: 'secret raw' })
		);
	});
	...
	it('falls back to entity-only extraction when the bundle returns zero mentions and a retry is warranted', async () => {
		...
		loadEntityGraphEnrichmentContextMock.mockResolvedValue({ communityExcerpts: [], graphEntities: [] });
	...
	it('marks the queue row failed and rethrows when the thought row is missing', async () => {
		...
		await expect(enrichQueuedThought('u1', 'missing')).rejects.toThrow('Queued thought not found: missing');
