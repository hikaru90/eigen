...
describe('editStoredThought', () => {
  beforeEach(() => {
    ...
    resolveThoughtCategoryMock.mockResolvedValue({
      key: 'task',
      ontologyEntityKindId: 'ek-1',
      confidence: 0.9,
      alternatives: [],
    })
    ...
    applyThoughtEditRequestMock.mockImplementation(
      async (input: { existingRawText: string; editRequest: string }) => ({
  ...
  it('merges metadata when existing row has null decrypted metadata', async () => {
    ...
    const updated = {
      ...existing,
      metadata: { status: 'completed', lastEditSummary: 'Marked complete' },
    }
