...
export async function syncCaptureEnrichQueue(userId: string): Promise<SyncCaptureEnrichQueueResult> {
	...
	const { isCaptureEnrichWorkerActive } = await import(
		'$lib/server/capture/capture-enrich-worker'
	);
...
export async function syncAndScheduleCaptureEnrichQueue(
	...
	if (
		result.activeThoughtIds.length > 0 &&
		(await shouldScheduleDevCaptureEnrichWorker(userId))
	) {
		const { scheduleCaptureEnrichWorker } = await import(
			'$lib/server/capture/capture-enrich-worker'
		);
