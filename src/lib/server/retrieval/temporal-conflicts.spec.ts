...
describe('findTemporalSchedulingConflictsInPostgres', () => {
  ...
  it('returns a conflict when overlapping events share a person but different places', async () => {
    ...
    expect(conflicts[0]!.mandatoryThoughtIds).toEqual(['t3'])
  ...
  it('includes other person-mentioning thoughts as mandatoryThoughtIds (excludes conflict pair)', async () => {
    ...
    // DB returns conflict pair ids plus a third mention — JS filter must drop t1/t2
    const mandatorySelect = vi.fn().mockReturnValue({
      from: vi.fn().mockReturnValue({
        innerJoin: vi.fn().mockReturnValue({
          where: vi
    ...
    expect(conflicts).toHaveLength(1)
    expect(conflicts[0]!.mandatoryThoughtIds).toEqual(['t-extra'])
    expect(conflicts[0]!.thoughtIds).toEqual(expect.arrayContaining(['t1', 't2', 't-extra']))
  })
