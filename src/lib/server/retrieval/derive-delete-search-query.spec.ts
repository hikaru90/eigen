...
describe('parseDeleteSearchQueriesResponse', () => {
  it('parses query array', () => {
    expect(
      parseDeleteSearchQueriesResponse('{"queries":["recipes cooking dishes","food preparation"]}'),
  ...
  it('returns LLM-derived content queries for a bulk recipe delete', async () => {
    llmChatCompletionMock.mockResolvedValue({
      choices: [
        {
          message: {
            content: '{"queries":["recipes cooking dishes meals","food ingredients preparation"]}',
          },
        },
      ],
    })
