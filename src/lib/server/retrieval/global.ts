...
export async function hasCommunitySummaries(userId: string): Promise<boolean> {
  ...
  const rows = await db
    ...
    .where(and(eq(communitySummary.userId, userId), isNotNull(communitySummary.summaryEmbedding)))
    .limit(1)
...
export async function searchGlobal(params: {
  ...
  if (candidates.length === 0) {
    return {
      answer:
        "I don't have enough memory clusters to answer this question yet. Try capturing more thoughts first.",
      communitiesUsed: 0,
      sources: [],
    }
  }
  ...
  const finalAnswer =
    Array.isArray(reduceChoices) && reduceChoices.length > 0
      ? (
          ((reduceChoices[0] as { message?: { content?: unknown } }).message?.content as string) ??
          ''
        ).trim()
