...
const MULTI_ANCHOR_KINDS: TemporalQuestionKind[] = ['ordering', 'duration', 'span', 'multi_ordering'];
...
export function parseTemporalHintBindingsResponse(
	...
	for (let i = 0; i < hints.length; i++) {
		...
		if (candidate.eventId !== eventId || candidate.thoughtId !== thoughtId) {
			throw new Error('temporal hint bindings: eventId and thoughtId must refer to the same candidate');
		}
...
export function candidatesFromTemporalSeeds(
	...
	return seeds.map((seed) => ({
		...
		startAt: seed.startAt && !Number.isNaN(seed.startAt.getTime()) ? seed.startAt.toISOString() : null,
		kind: seed.kind
	}));
}
