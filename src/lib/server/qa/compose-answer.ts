...
export function formatThoughtsForPrompt(items: RetrievalContextItem[], now: Date): string {
  ...
  return items
    .map((t) => {
      ...
      const staleFlag = t.isStale
        ? ' ⚠ STALE (>6 months old — may no longer reflect current state)'
        : ''
...
export function formatCommunityThemesForPrompt(themes: RelevantCommunitySummary[]): string {
  ...
  const lines = themes.map((t, i) => `[theme-${i + 1}] (level ${t.level}) ${t.summaryText.trim()}`)
  return (
...
export function insufficientMemoryAnswer(question: string): string {
  return ['Answer: Not in memory.', 'Evidence:', 'Unknown:', `- ${question.trim()}`].join('\n')
}
...
export function findInvalidProfileCitationIds(answer: string, allowedIds: Set<string>): string[] {
  ...
  if (!details) return []
  return findInvalidCitationIds(details, allowedIds).filter(
    (id) => !PROFILE_CITATION_PLACEHOLDERS.has(id),
  )
}
...
export async function composeAnswer(input: ComposeAnswerInput): Promise<ComposedAnswer> {
  ...
  const retrievalExtras: RetrieveEvidenceExtras = {
    communityThemes: [],
    temporalSeeds: [],
  }
  const [searchResults, textFileHits] = await Promise.all([
    searchThoughts({
      userId: input.userId,
      ...
      extrasOut: retrievalExtras,
    }),
    searchTextFiles(input.userId, { query: retrievalQuery, topK: TEXT_FILE_COMPOSE_TOP_K }),
  ])
  const communityThemes = scope === 'global' ? retrievalExtras.communityThemes.slice(0, 6) : []
  ...
  const temporalFetch =
    queryIntent.temporal && TEMPORAL_SOLVER_KINDS.includes(queryIntent.kind)
      ? await (async (): Promise<TemporalSeedsFetchResult> => {
          const usePerHint = shouldUsePerHintTemporalSeeds(queryIntent.kind, entityHints.length)
          // Reuse seeds from retrieveEvidence when they cover the non-per-hint path.
          if (
            !usePerHint &&
            queryIntent.kind !== 'count' &&
            retrievalExtras.temporalSeeds.length > 0
          ) {
            return { seeds: retrievalExtras.temporalSeeds, candidatesByHint: [] }
          }
          return fetchTemporalSeedsForCompose({
            userId: input.userId,
            query: retrievalQuery,
            queryEmbedding,
            entityHints,
  ...
  if (solverResult.confidence === 'low' && entityHints.length >= 1 && temporalSeeds.length > 0) {
  ...
  if (
    ...
    console.warn('[composeAnswer] stripping disallowed computed timeline citations from LLM answer')
