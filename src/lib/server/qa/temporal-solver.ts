...
import type { DurationUnit, TemporalQuestionKind } from '$lib/server/retrieval/classify-query-intent';
...
export function formatSolverAnswer(result: TemporalSolverResult): string | null {
	...
	if (result.kind === 'multi_ordering' && result.multiOrdering) {
		...
		const orderLine = ordered
			.map((e) => `"${e.label}" (${formatIsoDate(e.startAt)})`)
			.join(' → ');
		return [
			`Answer: ${orderLine} [id=${COMPUTED_TIMELINE_CITATION_ID}].`,
			'Evidence:',
			...ordered.map(
				(e) => `- ${e.label} (${formatIsoDate(e.startAt)}) [id=${e.thoughtId}]`
			),
