...
export const ontologyEntityKind = pgTable(
  'ontology_entity_kind',
  {
    ...
    /**
     * Durable knowledge: age alone never makes thoughts of this kind stale.
     * Seeded for seeded thought_category kinds; defaults false for new/custom kinds.
     */
    neverStale: boolean('never_stale').notNull().default(false),
...
export const captureSession = pgTable(
  'capture_session',
  {
    ...
    metadataPreview: jsonb('metadata_preview')
    ...
    authorKeyId: uuid('author_key_id').references(() => userApiKey.id, { onDelete: 'set null' }),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at')
      .defaultNow()
      .$onUpdate(() => new Date())
 ...
 */
/**
 * Structured memory type — DEPRECATED and removed in the single-type-axis migration (0067).
 * All usage sites must be cleaned up; `thought.category` is the single type axis.
 *
...
export const thoughtRelation = pgTable(
  'thought_relation',
  {
    ...
    ontologyRelationKindId: uuid('ontology_relation_kind_id').references(
      () => ontologyRelationKind.id,
      {
        onDelete: 'set null',
      },
    ),
...
export const userPreference = pgTable(
  'user_preference',
  {
    ...
    preferredTranscriptionQuality: text('preferred_transcription_quality').notNull().default('low'),
...
export const userGroundingProfile = pgTable('user_grounding_profile', {
  ...
  /** When a grounding/check-in push was last sent (shared 24h cap). */
  lastGroundingPushAt: timestamp('last_grounding_push_at', { withTimezone: true }),
  /** Persisted check-in question from the last push, served on notification tap. */
  pendingCheckIn: jsonb('pending_check_in').$type<Record<string, unknown> | null>(),
  updatedAt: timestamp('updated_at')
    .defaultNow()
    .$onUpdate(() => new Date())
    .notNull(),
...
export const evalEntry = pgTable(
  'eval_entry',
  {
    ...
    error: text('error'),
    durationMs: integer('duration_ms'),
    dependsOnEntryId: uuid('depends_on_entry_id').references((): AnyPgColumn => evalEntry.id, {
      onDelete: 'set null',
    }),
    startedAt: timestamp('started_at'),
    finishedAt: timestamp('finished_at'),
...
/**
 * User-submitted product feedback. Tenant-scoped via `user_id` (RLS); content is
 * stored verbatim — no semantic classification is applied at ingest (see AGENTS.md).
 */
export const feedback = pgTable(
  ...
  (t) => [index('feedback_user_idx').on(t.userId), index('feedback_created_idx').on(t.createdAt)],
)
