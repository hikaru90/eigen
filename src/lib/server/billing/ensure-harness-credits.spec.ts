...
describe('ensureHarnessWalletCredits', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		withDbUserMock.mockImplementation(async (_userId: string, fn: (db: unknown) => Promise<unknown>) =>
		...
		creditFromPaymentMock.mockResolvedValue({ credited: true, availableCredits: HARNESS_TEST_TOP_UP_CREDITS });
	});
	...
	it('skips top-up when operator wallet already has credits', async () => {
		...
		expect(result).toEqual({ billingUserId: GRAPH_SCALE_OPERATOR_USER_ID, availableCredits: 10_000 });
	...
	it('tops up graph-scale operator when corpus tenant has no wallet', async () => {
		...
		expect(withDbUserMock).toHaveBeenCalledWith('graph-scale-corpus-run-uuid-1', expect.any(Function));
	...
	it('respects custom minimum threshold', async () => {
		...
		const { ensureHarnessWalletCredits } = await import('./ensure-harness-credits');
		await ensureHarnessWalletCredits('graph-scale-runner', { minCredits: MIN_CAPTURE_PIPELINE_CREDITS + 1 });
