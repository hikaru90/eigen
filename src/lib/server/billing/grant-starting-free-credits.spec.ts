...
function makeGrantMocks(options: {
	...
	const userSelectLimit = vi.fn().mockResolvedValue([
		{ accountKind: options.accountKind ?? 'production' }
	]);
	const bonusSelectLimit = vi.fn().mockResolvedValue(options.existingBonus ? [{ id: 'bonus-1' }] : []);
	const walletSelectFor = vi.fn().mockResolvedValue([wallet]);
	const walletSelectLimit = vi.fn().mockResolvedValue([{ availableCredits: wallet.availableCredits }]);
	...
	withDbUserMock.mockImplementation(async (_userId: string, fn: (db: typeof db) => Promise<unknown>) =>
		fn(db)
	);
