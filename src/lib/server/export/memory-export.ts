...
const ENTITY_ALIASES_CSV_HEADERS = [
  'id',
  'canonical_entity_id',
  'alias_text',
  'created_at',
...
async function buildThoughtEntitiesCsv(userId: string): Promise<string> {
  const rows = await getDb()
    ...
    .orderBy(
      asc(thoughtEntity.createdAt),
      asc(thoughtEntity.thoughtId),
      asc(thoughtEntity.entityId),
    )
...
async function buildThoughtTextFileCsv(userId: string): Promise<string> {
  ...
  return buildCsv(
    ...
    rows.map((row) => [row.thoughtId, row.textFileId, formatTimestamp(row.createdAt)]),
  )
}
