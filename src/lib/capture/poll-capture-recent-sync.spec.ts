...
describe('pollCaptureRecentSync', () => {
  ...
  it('passes authorLayerKey to fetch when user view is an agent', async () => {
    ...
    await vi.advanceTimersByTimeAsync(0)
    expect(fetchMock).toHaveBeenCalledWith('/api/capture/recent?authorLayerKey=apikey%3Akey-1', {
      cache: 'no-store',
    })
    cancel()
  })
  ...
  it('passes author=user to fetch when user view is "user"', async () => {
    ...
    await vi.advanceTimersByTimeAsync(0)
    expect(fetchMock).toHaveBeenCalledWith('/api/capture/recent?author=user', { cache: 'no-store' })
    cancel()
  })
})
