...
function thought(
	overrides: Partial<Pick<CaptureSubmitResult, 'enrichmentComplete' | 'queueStatus' | 'queueError'>>
): CaptureSubmitResult {
	return {
		id: 't1',
...
describe('captureIndexingListStatus', () => {
	it('returns null when enrichment is complete', () => {
		expect(
			captureIndexingListStatus(thought({ enrichmentComplete: true, queueStatus: 'complete' }), true)
		).toBeNull();
	});
