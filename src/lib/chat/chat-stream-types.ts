...
export const CHAT_TOOL_COPY: Record<string, ChatToolVisual> = {
  ...
  list_projects: { title: 'Listing projects', category: 'memory', icon: 'list' },
  get_project_timeline: { title: 'Opening project timeline', category: 'memory', icon: 'list' },
  ...
  unlink_text_file_from_thought: {
    title: 'Unlinking note from memory',
    category: 'write',
    icon: 'pencil',
  },
}
...
export function formatToolArgumentsSummary(
  ...
  if (tool === 'edit_thought') {
    ...
    const instruction = args.edit_request ?? args.instruction ?? args.edit ?? args.request
    const parts: string[] = []
