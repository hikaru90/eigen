...
describe('normalizeCitationTokens', () => {
	...
	it('rewrites [<id=uuid>] citations to [id=uuid]', () => {
		expect(
			normalizeCitationTokens(
				'You are home. [<id=d2af9064-8fbe-490a-856a-ccaee8410516>]'
			)
...
describe('stripCitationTokens', () => {
	...
	it('extracts ids from [<id=uuid>] citations', () => {
		expect(
			extractCitationIds('home office [<id=d2af9064-8fbe-490a-856a-ccaee8410516>]')
		).toEqual(['d2af9064-8fbe-490a-856a-ccaee8410516']);
	});

	it('extracts ids from [id=uuid] citations', () => {
		expect(
			extractCitationIds('Respond tonight [id=d428954a-aae1-4565-a162-9f38b5536d2e]')
		).toEqual(['d428954a-aae1-4565-a162-9f38b5536d2e']);
	});
});
