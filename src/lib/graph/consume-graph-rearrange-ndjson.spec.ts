...
describe('consumeGraphRearrangeNdjsonStream', () => {
  it('streams progress phases and returns the final result', async () => {
    ...
    const body = new ReadableStream({
      start(controller) {
        controller.enqueue(encoder.encode('{"type":"progress","phase":"prune_weak_edges"}\n'))
        controller.enqueue(encoder.encode('{"type":"progress","phase":"repair_relations"}\n'))
        controller.enqueue(
          encoder.encode(
    ...
    expect(events).toEqual([{ phase: 'prune_weak_edges' }, { phase: 'repair_relations' }])
