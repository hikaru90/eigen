<script lang="ts">
  ...
  let agents = $state<PageData['agents']>(data.agents)
  let projects = $state<PageData['projects']>(data.projects)
  let agentProjectBindings = $state<
    Record<string, Array<{ projectEntityId: string; projectLabel: string }>>
  >(data.agentProjectBindings)
...
<div class="mx-auto max-w-xl space-y-6 px-5 pb-8 pt-10">
  ...
  <div class="rounded-sm border border-black/10 px-4 py-3 dark:border-white/10 space-y-2">
    ...
    <p class="text-muted-foreground text-[11px] leading-relaxed">
      ...
      Agents report completion to <code class="font-mono text-[10px]"
        >POST /api/agents/callback/complete</code
      >
            ...
            <div class="rounded-sm border border-black/10 px-3 py-2 dark:border-white/10">
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0 flex-1">
                  ...
                  {#if agentProjectBindings[agent.id]?.length > 0}
                    <div class="mt-1 flex flex-wrap gap-1">
                      {#each agentProjectBindings[agent.id] as binding (binding.projectEntityId)}
                        <span
                          class="inline-flex items-center gap-0.5 rounded-sm bg-blue-50 px-1.5 py-0.5 text-[9px] text-blue-700 dark:bg-blue-900/30 dark:text-blue-300"
                        >
                          ...
                          <button
                            ...
                            onclick={() =>
                              void toggleAgentProject(agent.id, binding.projectEntityId, true)}
                          >
                            <X class="size-2.5" />
                          </button>
                        </span>
                  ...
                  {:else}
                    <p class="text-muted-foreground mt-1 text-[9px] italic">
      ...
      <div class="space-y-3">
        ...
        {#if projects.length > 0}
          <div class="space-y-2">
            ...
            <p class="text-muted-foreground text-[10px]">
              If no projects selected, agent receives all events.
            </p>
              ...
              {#each projects as project (project.projectEntityId)}
                <label class="flex items-center gap-2 text-xs">
                  <input
                    ...
                    onchange={(e) =>
                      toggleProject(project.projectEntityId, e.currentTarget.checked)}
                  />
    ...
    {:else}
      ...
      <div class="space-y-3">
        <div>
          ...
          <div class="relative">
            <code class="block break-all rounded-sm border px-3 py-2 pr-9 text-[10px]"
              >{generatedSecrets.signingSecret}</code
            >
            <button
              type="button"
            ...
            >
              {#if copiedField === 'signing'}<Check class="size-4 text-green-500" />{:else}<CopyIcon
                  class="size-4"
                />{/if}
            </button>
          </div>
        ...
        <div>
          <p class="text-[10px] text-muted-foreground mb-1">
            Callback token (complete assignments)
          </p>
          <div class="relative">
            <code class="block break-all rounded-sm border px-3 py-2 pr-9 text-[10px]"
              >{generatedSecrets.callbackToken}</code
            >
            <button
              type="button"
              class="absolute right-1 top-1 p-1"
            ...
            >
              {#if copiedField === 'callback'}<Check
                  class="size-4 text-green-500"
                />{:else}<CopyIcon class="size-4" />{/if}
            </button>
          </div>
        </div>
      </div>
  ...
  <Dialog.Content class="max-w-md rounded-[4px]">
    ...
    <div class="space-y-2">
      ...
      {:else}
        {#each projects as project (project.projectEntityId)}
          {@const bound =
            agentProjectBindings[bindingAgentId ?? '']?.some(
