<script lang="ts">
  ...
  function stepDetail(jobId: string, run: DisplayRun, task: TaskRow): string | null {
    const completed = run.jobs.find((j) => j.job === jobId)
  ...
  function stepReport(jobId: string, run: DisplayRun, task: TaskRow) {
    ...
    const liveDetail =
      jobId === 'community_summaries' && !completed ? stepDetail(jobId, run, task) : null
  ...
  async function retryFailedStep(task: TaskRow, run: DisplayRun, jobId: string) {
    ...
    try {
      ...
      actionMessage =
        body.message ??
        (body.job?.ok
          ? `Retried ${jobLabel(jobId)} successfully.`
          : `Retry of ${jobLabel(jobId)} failed again.`)
    } catch (err) {
  ...
  async function runNow(task: TaskRow) {
    ...
    try {
      ...
      const plannedJobs = body.plannedJobs?.length ? body.plannedJobs : []
      trackedRun = {
    ...
    try {
      ...
      if (!body.softCancelled) {
        ...
        trackedRun = trackedRun
          ? {
              ...trackedRun,
              finishedAt: trackedRun.finishedAt ?? Date.now(),
              cancelRequested: true,
            }
          : null
        stopPolling()
...
<main class="mx-auto max-w-lg px-5 pb-16 pt-24">
  ...
  <p class="text-muted-foreground mt-1 text-sm leading-relaxed">
    ...
    progress (work already finished is kept). Pause schedule only affects the overnight timer — not
    a mid-run stop.
  </p>
                        ...
                        {#if expanded}
                          ...
                          >
                            ...
                            {#if report.samples && report.samples.length > 0}
                              <p
                                class="text-foreground text-[10px] font-medium tracking-wide uppercase"
                              >
                                What changed
                              </p>
                              <ul class="space-y-1">
                                {#each report.samples as sample, i (sample.id ?? `${sample.label}-${i}`)}
                                  <li class="text-muted-foreground text-[11px] leading-snug">
                                    {#if sample.kind === 'thought' && sample.id}
                                      <a
                                        href="{resolve('/memory')}?thought={encodeURIComponent(
                                          sample.id,
                                        )}"
