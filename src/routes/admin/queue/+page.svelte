<script lang="ts">
	...
	async function refreshDashboard() {
		...
		try {
			...
			if (!res.ok) {
				throw new Error(typeof body?.message === 'string' ? body.message : `Refresh failed (${res.status})`);
			}
...
<div class="mx-auto max-w-6xl px-5 pb-8 pt-10">
	...
	<div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
		<Card.Root class="border border-black/10 bg-card ring-0 shadow-[4px_4px_0_0_rgb(17_17_17_/_0.08)]">
		...
		<Card.Root class="border border-black/10 bg-card ring-0 shadow-[4px_4px_0_0_rgb(17_17_17_/_0.08)]">
			...
			<Card.Content class="space-y-2 text-xs">
				<div class="flex justify-between gap-4">
					...
					<span class={ops.push.vapidConfigured ? 'text-emerald-700 dark:text-emerald-400' : 'text-destructive'}>
					...
					<ul class="space-y-1">
						{#each ops.pgCronJobs as job (job.jobName)}
							<li class="flex justify-between gap-4">
								...
								<span class="text-muted-foreground">{job.schedule} · {job.active ? 'active' : 'inactive'}</span>
							</li>
						{/each}
					</ul>
				{/if}
						...
						<ul class="space-y-1 font-mono text-[11px]">
							{#each ops.recentPgNetHttp as row (row.id)}
								<li class="flex justify-between gap-2">
									<span>#{row.id}</span>
									<span class={row.statusCode && row.statusCode >= 200 && row.statusCode < 300 ? 'text-emerald-700 dark:text-emerald-400' : 'text-destructive'}>
										{row.statusCode ?? '—'}
									</span>
	...
	<Card.Root class="mt-4 border border-black/10 bg-card ring-0 shadow-[4px_4px_0_0_rgb(17_17_17_/_0.08)]">
		<Card.Header class="flex flex-row items-center gap-2 pb-3">
			...
			<div>
				...
				<Card.Description>Webhook deliveries, overnight consolidation, and other per-user background jobs</Card.Description>
			</div>
		</Card.Header>
						...
						<Table.Body>
							{#each dashboard.jobs as job (job.id)}
								<Table.Row>
									...
									<Table.Cell class="max-w-[10rem] truncate text-xs" title={job.userEmail ?? job.userId}>
										{job.userEmail ?? job.userId}
										{#if job.accountKind !== 'production'}
											<span class="text-muted-foreground"> ({job.accountKind})</span>
										{/if}
									</Table.Cell>
									<Table.Cell class="text-xs whitespace-nowrap">{formatWhen(job.runAfter)}</Table.Cell>
									<Table.Cell class="font-mono text-xs tabular-nums">
										{job.attemptCount}/{job.maxAttempts}
									</Table.Cell>
									<Table.Cell class="max-w-[8rem] truncate font-mono text-[10px]" title={job.dedupeKey ?? ''}>
										{job.dedupeKey ?? '—'}
									</Table.Cell>
									<Table.Cell class="max-w-[14rem] truncate text-[11px] text-destructive" title={job.lastError ?? ''}>
										{job.lastError ?? '—'}
									</Table.Cell>
								</Table.Row>
							{/each}
