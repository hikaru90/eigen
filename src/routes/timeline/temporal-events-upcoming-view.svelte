<script lang="ts">
  ...
  const projectGroups = $derived(groupByProject(viewItems, m.graph_timeline_no_project(), timeZone))
</script>

<div
  class="min-h-0 flex-1 overflow-y-auto pb-28"
  role="listbox"
  aria-label={m.graph_timeline_upcoming_aria()}
>
  {#if projectGroups.length === 0}
    <p class="text-muted-foreground px-4 py-8 text-center text-sm">
      {m.graph_timeline_upcoming_empty()}
    </p>
  {:else}
