<script lang="ts">
  ...
  import TemporalEventsTodayView from './temporal-events-today-view.svelte'
  import TemporalEventsProjectsView from './temporal-events-projects-view.svelte'
  ...
  const initialRange = { from: null, to: null, includeUndated: true, label: 'All time' }
  ...
  const filtersActive = $derived(statusFilter !== 'open')
  ...
  const filteredItems = $derived(
    phase.kind === 'ready' ? filterItemsByStatus(phase.items, statusFilter) : [],
  )
  ...
  const todayTodoItems = $derived(
    filterRangeScopedTodoItems(todayTodoSourceItems, new Date(), statusFilter),
  )

  /** Same list Tasks renders — Projects only groupBy. */
  const sharedProjectItems = $derived(todayTodoItems)
  ...
  function appendAbsoluteRange(params: SvelteURLSearchParams) {
    // Always send from/to so All time (null/null) still uses absolute mode, not legacy relevant.
    params.set('from', dateRange.from ?? '')
    params.set('to', dateRange.to ?? '')
    params.set('includeUndated', dateRange.includeUndated ? 'true' : 'false')
  }
  ...
  async function loadOverdueItems(options?: { silent?: boolean }) {
    ...
    try {
      ...
      overdueItems = filterPriorDayOverdueItems(filterActiveItems(body.items), userTimeZone)
    } catch {
      overdueItems = []
    } finally {
  ...
  async function loadDoneItems(options?: { silent?: boolean }) {
    ...
    try {
      ...
      const body = (await res.json()) as { items: TemporalEventListItem[] }
      doneItems = filterCompletedTodayItems(filterActiveItems(body.items), userTimeZone)
    } catch {
      doneItems = []
    } finally {
      doneLoading = false
  ...
  function applyProjectLabelLocally(
    thoughtId: string,
    projectLabel: string,
    ...
    {#snippet titleActions()}
      ...
      >
        <TimelineDateRangeDial
          from={dateRange.from}
          to={dateRange.to}
          includeUndated={dateRange.includeUndated}
