...
describe('timeline quick action contract', () => {
	...
	it('postTimelineQuickAction hits temporal-events action for a task id', async () => {
		...
		const result = await postTimelineQuickAction('task:t1', 'mark_done', fetchImpl as unknown as typeof fetch);
...
describe('timeline quick action wiring (no forked mark-done clients)', () => {
	it('TemporalEvents and ProjectsView both import postTimelineQuickAction', () => {
		const tasks = readTimeline('TemporalEvents.svelte');
		const projects = readTimeline('TemporalEventsProjectsView.svelte');
		expect(tasks).toContain("from './timeline-item-actions'");
		expect(tasks).toContain('postTimelineQuickAction');
		expect(projects).toContain("from './timeline-item-actions'");
		expect(projects).toContain('postTimelineQuickAction');
	});

	it('ProjectsView does not fork a thoughts PATCH mark-done path', () => {
		const projects = readTimeline('TemporalEventsProjectsView.svelte');
		expect(projects).not.toMatch(/fetch\(`\/api\/thoughts\/\$\{/);
		expect(projects).not.toContain('thoughtIdFromTaskItemId(itemId)');
	});
