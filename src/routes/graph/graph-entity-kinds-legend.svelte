<script lang="ts">
	...
	import { graphFilterGlassPanelClass, graphFilterTriggerClass } from '$lib/graph/graph-filter-chrome';
	...
	const shellClass = $derived(
		`${graphFilterGlassPanelClass(legendPanelExpanded)} pointer-events-auto flex flex-col gap-0.5 overflow-hidden p-0.5 ${
			legendPanelExpanded
				? 'w-full max-w-[min(calc(100vw-1.5rem),11rem)] max-h-56'
				: 'w-fit'
		}`
	);
...
{#if showLegend}
	<aside class={shellClass} aria-label={m.graph_aria_entity_type_filter()}>
		...
		>
			...
			>
				<ListFilterIcon class="size-2.5 shrink-0 opacity-90" strokeWidth={1.75} aria-hidden="true" />
