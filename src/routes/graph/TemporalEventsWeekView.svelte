						...
						{#each placements.filter((p) => p.dayIndex === dayIndex) as p (`${p.item.id}-${p.startMinutes}`)}
							<button
								...
								p.item.id
									? 'ring-primary ring-2'
									: ''}"
								style="top: {(p.startMinutes - WEEK_GRID_START_HOUR * 60) * pxPerMinute}px; height: {Math.max(
									p.durationMinutes * pxPerMinute,
									18
								)}px; background-color: {kindColor(p.item.kind)}22; border-color: {kindColor(p.item.kind)}"
								draggable="true"
								ondragstart={() => (dragEventId = p.item.id)}
								onclick={() => onSelect(p.item)}
							>
								<span class="text-foreground line-clamp-2 font-medium">{p.item.semanticSummary}</span>
							</button>
						{/each}
					</div>
				{/each}
