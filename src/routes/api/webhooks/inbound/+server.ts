...
export const POST: RequestHandler = async (event) => {
	...
	const [subscription] = await db
		...
		.where(and(
			...
			eq(inboundWebhookSubscription.enabled, true)
		))
		.limit(1);
	...
	if (signatureMode === 'github') {
		...
		eventType = event.request.headers.get('x-github-event')
		...
		eventType = event.request.headers.get('x-gitlab-event')
			?? (typeof payload.event === 'string' ? payload.event : 'webhook.received');
		deliveryId = undefined;
	} else {
		// Generic: event type from X-Event-Type, delivery ID from X-Request-ID
		eventType = event.request.headers.get('x-event-type')
			?? (typeof payload.event === 'string' ? payload.event : 'webhook.received');
	...
	return json({ received: true, eventType, deliveryId, agentRunId } satisfies InboundWebhookResponse);
};
