<script lang="ts">
  ...
  const vizCtx = {
    ...
    get selectedCommunityLevel(): number | null {
      const parsed = Number.parseInt(graphFilters.communityLevel, 10)
  ...
  let embeddingMapApi = $state<EmbeddingMapApi | null>(null)

  afterNavigate(() => {
    if (page.url.searchParams.get('view') === 'embeddings') {
      embeddingsTabOpened = true
      queueMicrotask(() => embeddingMapApi?.resize())
    }
  })
  ...
  const nodeDrawerOpen = $derived(selectedNode !== null || selectedCommunity !== null)
  ...
  function beginThoughtEdit(thoughtId: string) {
    blurActiveElement()
    void loadThoughtEditor(thoughtId)
  }
  ...
  function clearThoughtEditorState() {
    thoughtEditorDraft = ''
  ...
  function loadThoughtEditor(id: string | null) {
    editingThoughtId = id
    if (!id) {
      clearThoughtEditorState()
      return
    }
  ...
  function loadEntityEditor(id: string) {
  ...
  function syncEditorsForSelectedNode(n: (typeof data.snapshot.nodes)[number] | null) {
    if (n?.kind === 'Thought') {
      clearEntityEditorState()
      loadThoughtEditor(n.id)
      return
    }
    loadThoughtEditor(null)
    if (!n || n.kind !== 'Entity') {
      clearEntityEditorState()
      return
    }
    loadEntityEditor(n.id)
  }

  function applyNodeSelection(
    n: (typeof data.snapshot.nodes)[number] | null,
    opts?: { restoreZoomOnClear?: boolean },
  ) {
    const restoreZoom = opts?.restoreZoomOnClear !== false
    selectedNode = n
    selectedCommunityId = null
    const id = n?.id ?? null
    queueMicrotask(() => scheduleApplyHighlight?.(id))
    embeddingMapApi?.setSelectedId(id)
    if (n === null && restoreZoom) {
      queueMicrotask(() => scheduleRestorePreEntityZoom?.())
    }
    syncEditorsForSelectedNode(n)
  }
  ...
  async function refreshGraphAfterEnrichment() {
    ...
    try {
      ...
      await invalidateAll()
      applyPendingPopInAfterDataRefresh()
  ...
  function onFiltersChanged() {
    normalizeCommunityLevelSelection()
    queueMicrotask(() => scheduleGraphUpdate?.())
    embeddingMapApi?.setVisibleSubtypes(graphFilters.visibleEntityTypes)
    embeddingMapApi?.setVisibleAuthorLayers(graphFilters.visibleAuthorLayers)
  }

  function applyPendingPopInAfterDataRefresh() {
    if (entityIdsBeforeEnrichRefresh) {
      const after = new Set(data.snapshot.nodes.filter((n) => n.kind === 'Entity').map((n) => n.id))
      pendingPopInNodeIds = new Set(
        [...after].filter((id) => !entityIdsBeforeEnrichRefresh!.has(id)),
      )
  ...
  function normalizeCommunityLevelSelection() {
    const levels = availableCommunityLevels
    if (graphFilters.communityLevel === 'leaf') {
      graphFilters.communityLevel = String(COMMUNITY_LEAF_LEVEL)
    } else if (
      levels.length > 0 &&
      !levels.some((l) => String(l) === graphFilters.communityLevel)
    ) {
      graphFilters.communityLevel = String(levels[0])
    }
  ...
  onMount(() => {
    const unsubscribeView = subscribeCurrentUserView((view) => {
      graphFilters.visibleAuthorLayers = new SvelteSet(viewToVisibleAuthorLayers(view))
      onFiltersChanged()
    })
    ...
    ;(async () => {
      ...
      const graphTooltip = d3
        ...
        .attr(
          'class',
          'graph-node-tooltip pointer-events-none absolute z-10 hidden max-w-[14rem] truncate whitespace-pre-line rounded border border-border/70 bg-background/90 px-1.5 py-0.5 font-mono text-[10px] text-foreground shadow-sm',
        )
      ...
      function applyLabelFontScale(k: number) {
        ...
        gCommunityChrome
          .selectAll<SVGTextElement, CommunityHull>('text.community-hull-label')
          .style('font-size', `${hullLabelPx}px`)
        gClusterMarkers
          .selectAll<SVGTextElement, GraphZoomCluster>('text.graph-cluster-label')
          .style('font-size', `${clusterLabelPx}px`)
        gClusterMarkers
          .selectAll<SVGTextElement, GraphZoomCluster>('text.graph-cluster-count')
          .style('font-size', `${clusterCountPx}px`)
      }

      function resizeSummaryLabelBackgrounds() {
        gCommunityChrome
          .selectAll<SVGGElement, CommunityHull>('g.community-hull-label-wrap')
          .each(function () {
        ...
        gClusterMarkers
          .selectAll<SVGGElement, GraphZoomCluster>('g.graph-cluster-label-wrap')
          .each(function () {
            const wrap = d3.select(this)
      ...
      function clusterRebuildKey(scale: number, nodes: SimNode[]): string {
        const levels = canonicalCommunityLevels((vizCtx.communities ?? []).map((c) => c.level))
      ...
      function onClusterClick(event: MouseEvent, cluster: GraphZoomCluster) {
        ...
        loadThoughtEditor(null)
        clearEntityEditorState()
        queueMicrotask(() => scheduleApplyHighlight?.(null))
        embeddingMapApi?.setSelectedId(null)
        queueMicrotask(() => scheduleRestorePreEntityZoom?.())
      ...
      function onNodeClick(event: MouseEvent, d: SimNode) {
        ...
        applyNodeSelection(hit ?? null, { restoreZoomOnClear: false })
        if (hit) {
          if (!prev) {
            const svgEl = svg.node()
            if (svgEl) {
      ...
      function updateGraph() {
        ...
        const rawNodes: SimNode[] = filterNodesByAuthorLayers(
          ...
          graphFilters.visibleAuthorLayers,
        ).map((n) => simNodeFromSnapshot(n))
        const typeFiltered = filterNodesByEntityTypes(rawNodes, graphFilters.visibleEntityTypes)
        const q = norm(graphFilters.search)
        ...
        const edgeFilter = (e: (typeof vizCtx.snapshot.edges)[0]) => {
          if (graphFilters.edgeKind !== 'all' && e.kind !== graphFilters.edgeKind) return false
          if (!visibleIds.has(e.sourceId) || !visibleIds.has(e.targetId)) return false
          if (graphFilters.visibleAuthorLayers.size === 0) return true
          if (e.kind === 'entity_relation') return true
          const a = e.sourceId
          const b = e.targetId
          const edgeKey = a < b ? `${a}:${b}` : `${b}:${a}`
          const layers = vizCtx.coMentionEdgeLayerKeys[edgeKey] ?? []
          return layers.some((key) => graphFilters.visibleAuthorLayers.has(key))
        }
...
<div class="-mb-28 h-svh overflow-hidden overscroll-none">
      ...
      <Card.Content class="flex h-full min-h-0 flex-1 flex-col p-0">
        ...
        >
          <div class="relative h-full min-h-0 w-full flex-1 bg-transparent">
            ...
            >
              ...
              >
                <GraphFiltersToolbar
                  bind:search={graphFilters.search}
                  bind:edgeKind={graphFilters.edgeKind}
                  bind:communityLevel={graphFilters.communityLevel}
                  {availableCommunityLevels}
                  {legendSections}
                  bind:visibleEntityTypes={graphFilters.visibleEntityTypes}
                  onsubmit={() => scheduleGraphUpdate?.()}
        ...
        >
          {#if embeddingsTabOpened}
            <EmbeddingMap
              bind:api={embeddingMapApi}
              visible={activeTab === 'embeddings'}
              graphLegendSections={data.graphLegendSections ?? []}
              bind:visibleEntityTypes={graphFilters.visibleEntityTypes}
              bind:visibleAuthorLayers={graphFilters.visibleAuthorLayers}
  ...
  <MemorySurfaceDrawer open={nodeDrawerOpen} onOpenChange={onNodeDrawerOpenChange}>
    {#if selectedCommunity}
      <Drawer.Description class="sr-only">{m.graph_drawer_community_sr()}</Drawer.Description>
      <div class="flex min-h-0 flex-1 flex-col overflow-y-auto px-4 pt-2 pb-10" data-vaul-no-drag>
          ...
          <div class="mt-3 border-t border-black/5 pt-3 dark:border-white/10">
            ...
            {:else}
              ...
              <div class="mt-4 border-t border-black/5 pt-3 dark:border-white/10">
                <p
                  class="text-muted-foreground mb-2 text-[10px] font-medium tracking-wide uppercase"
                >
            ...
            {:else}
              ...
              <div class="space-y-2">
                ...
                <Textarea
                  ...
                  rows={4}
                  class="font-mono text-xs"
                  disabled={thoughtEditorBusy || thoughtEditorRelinkBusy || thoughtEditorDeleteBusy}
                />
              </div>
                ...
                {#if selectedNode?.kind === 'Entity'}
                  <Button
                    ...
                    disabled={thoughtEditorBusy ||
                      thoughtEditorRelinkBusy ||
                      thoughtEditorDeleteBusy}
                    onclick={() => loadThoughtEditor(null)}
                  >
                    {m.graph_cancel()}
                  </Button>
                {/if}
