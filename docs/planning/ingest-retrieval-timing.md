...
- **Semantic / vector search** — Text is turned into a **number vector** (embedding). Similar _meaning_ → vectors close together, even when words differ (“automobile” vs “car”). Requires tier-2 `embedding` on the thought row. Implemented as **ANN** (approximate nearest neighbor): a fast index over millions of vectors that finds “close enough” neighbors without scanning every row.
