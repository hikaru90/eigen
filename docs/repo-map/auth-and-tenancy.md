...
- **Purpose:** `betterAuth` instance: email/password, optional OAuth (`google`, `github` via [`src/lib/server/auth-social.ts`](../../src/lib/server/auth-social.ts) when env credentials are set), Drizzle adapter on `authDb`, `normalizeAuthOrigin(env.ORIGIN)`, cookies plugin. User records carry optional `firstName`/`lastName` additional fields (signup form inputs or OAuth `mapProfileToUser` + [`normalizeUserNameFields`](../../src/lib/server/auth-user-name.ts)); transactional email and onboarding list sync via Owlery ([`src/lib/server/owlery/mail.ts`](../../src/lib/server/owlery/mail.ts), [`src/lib/server/owlery/contacts.ts`](../../src/lib/server/owlery/contacts.ts)). `databaseHooks.user.create.after` grants starting credits and syncs the onboarding contact when `emailVerified`; email/password users also sync on `afterEmailVerification`.
