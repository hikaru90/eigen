...
if (!ageGraph) {
  console.error(
    '[eigen] ensure-extensions failed: AGE_GRAPH_NAME is required and must be non-empty.',
  )
...
try {
  ...
  await sql.unsafe(`ALTER DATABASE ${quoteIdent(dbName)} SET search_path TO ${AGE_SEARCH_PATH}`)
  console.log(
  ...
  console.error(
    '[eigen] Check that DATABASE_URL is correct, the database is accessible, and PostgreSQL has AGE loaded.',
  )
  process.exit(1)
} finally {
  await sql.end()
