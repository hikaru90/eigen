...
describe('drizzle migration journal', () => {
	it('lists every numbered migration SQL file (deploy runs journal entries only)', () => {
		...
		expect(missingFromJournal, `add to drizzle/meta/_journal.json: ${missingFromJournal.join(', ')}`).toEqual(
			[]
		);
	});
	...
	it('has a journal entry for every numbered migration SQL file on disk', () => {
		...
		expect(orphanJournal, `missing SQL files for journal tags: ${orphanJournal.join(', ')}`).toEqual([]);
	});
});
