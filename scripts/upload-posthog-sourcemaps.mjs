...
function resolvePostHogCliHost() {
  ...
  if (publicHost?.includes('posthog.com'))
    return publicHost.replace('.i.posthog.com', '.posthog.com')
...
console.log(
  `[posthog] Uploading source maps from ${directory} to ${host} (release ${releaseLabel})`,
)
