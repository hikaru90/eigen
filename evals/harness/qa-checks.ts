...
export async function runStructuralChecks(input: {
  ...
  for (const temporalCheck of checks.temporal ?? []) {
    ...
    const kindsOk =
      !temporalCheck.kinds?.length || rows.some((r) => temporalCheck.kinds!.includes(r.kind))
  ...
  for (const fixtureId of checks.embedding?.requireVector ?? []) {
    ...
    const dimOk =
      Array.isArray(vec) && vec.length === expectedDim && vec.every((n) => Number.isFinite(n))
