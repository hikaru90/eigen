...
export function buildRetrievalRelevantPrunePlan(input: {
  ...
  const pruneBroken = ingestBrokenEligibleForRetrievalPrune(input.ingestBrokenFixtureIds, needle)
  const gradedBroken = input.retrievalRelevant.filter((r) => pruneBroken.has(r.id)).map((r) => r.id)
  if (gradedBroken.length === 0) return null

  const { retrievalRelevant: retrievalRelevantAfter, removed } =
