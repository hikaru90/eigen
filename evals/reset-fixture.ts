...
async function findFixtureThoughts(fixtureId: string, operatorUserId?: string): Promise<Array<{ evalUserId: string; thoughtId: string }>> {
	if (operatorUserId) {
		const evalUserId = evalCorpusUserId(operatorUserId);
		const rows = await appSql<
			{ eval_user_id: string; thought_id: string }[]
		>`
...
async function main(): Promise<void> {
	...
	if (targets.length === 0) {
		logEval(`no mapped thought for fixture ${fixtureId}${operatorUserId ? ` (operator ${operatorUserId})` : ''}`);
		return;
	}
