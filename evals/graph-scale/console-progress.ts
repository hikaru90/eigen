...
export class GraphScaleConsoleProgress {
	...
	private seedPartialWeight(): number {
		return (
			(this.seedQueuePartial * SEED_QUEUE_FRACTION + this.seedEnrichPartial * SEED_ENRICH_FRACTION) *
			WEIGHTS.seed
		);
	}
	...
	report(
		...
		} else if (opts?.seedEnriched !== undefined && opts.seedEnrichTotal && opts.seedEnrichTotal > 0) {
	...
	print(force = false): GraphScaleProgressSnapshot {
		...
		const throttled =
			...
			!pctChanged &&
			!labelChanged &&
			now - this.lastPrintAt < MIN_PRINT_INTERVAL_MS;
