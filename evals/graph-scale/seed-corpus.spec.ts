...
import { buildCorpusTexts, buildRepeatedProbeTexts, buildSpendPicnicProbeTexts, graphScaleCorpusUserId, graphScaleSpendProbeText, GRAPH_SCALE_SPEND_PICNIC_PROBE, GRAPH_SCALE_SPEND_PICNIC_PROBE_MAX, overflowCaptureText } from './seed-corpus';
...
describe('graph-scale seed corpus', () => {
	...
	it('buildSpendPicnicProbeTexts returns linked picnic sequence', () => {
		...
		expect(texts.every((t) => t.toLowerCase().includes('picnic') || t.includes('fish'))).toBe(
			true
		);
	});
