...
export async function collectGraphScaleMetrics(
  ...
  const [thoughtRow, entityRow, communityRow, projectRow, entityEdges] = await Promise.all([
    database.select({ n: count() }).from(thought).where(eq(thought.userId, userId)),
    database.select({ n: count() }).from(canonicalEntity).where(eq(canonicalEntity.userId, userId)),
    database.select({ n: count() }).from(graphCommunity).where(eq(graphCommunity.userId, userId)),
    database
      .select({ n: count() })
