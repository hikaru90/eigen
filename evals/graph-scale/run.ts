...
async function main(): Promise<void> {
	...
	try {
		...
		for (let sizeIndex = 0; sizeIndex < cli.sizes.length; sizeIndex++) {
			...
			emitProgress(consoleProgress, fileProgress, `N=${n} seed (${sizeIndex + 1}/${cli.sizes.length})`);
			...
			if (cli.tracks.has('consolidation')) {
				...
				emitProgress(consoleProgress, fileProgress, `N=${n} consolidation done`, { bump: 'consolidation' });
			}
		...
		consoleProgress.finish(
			`[graph-scale] 100% · done · report ${reportPaths.jsonPath}`
		);
	} catch (err) {
