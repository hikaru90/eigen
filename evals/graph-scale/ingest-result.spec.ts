...
describe('isGraphScaleIngestOk', () => {
  it('requires enrich, embedding, and at least one entity', () => {
    expect(isGraphScaleIngestOk({ enriched: true, hasEmbedding: true, entityCount: 2 })).toBe(true)
    expect(isGraphScaleIngestOk({ enriched: true, hasEmbedding: true, entityCount: 0 })).toBe(false)
    expect(isGraphScaleIngestOk({ enriched: false, hasEmbedding: true, entityCount: 1 })).toBe(
      false,
    )
  })
})
